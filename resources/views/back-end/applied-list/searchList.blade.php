            ...
            <table id="table_id" class="display table table-bordered table-hover table-striped text-center">
                ...
                @if($applicants)
                    @foreach($applicants as $applicant)
                        <tr>
                            <td><input type="checkbox" name="checkSingle[]" id="singleCheckAll" value="{{ $applicant->id }}"/></td>
                            <td>{{ $applicant->unique_apply_id }}</td>
                            <td>{{ $applicant->post_name }}</td>
                            <td>@if($applicant->totalExperince == '') 0 @else {{ $applicant->totalExperince }} @endif</td>
                            <td>{{ $applicant->skills }}</td>
                            <td><img src="{{ asset($applicant->photo) }}" width="50" alt="Applicant Image"/></td>

                            <td>
                                <a href="{{ url('single-advertisement',['id' => $applicant->id ]) }}" class="btn btn-info btn-sm" title="View">
                                    <i class="fas fa-search-plus"></i>
                                </a>
                                <a onclick="return confirm('Are you sure to delete!'); "  href="{{ url('delete-advertisement',['id' => $applicant->id] ) }}" class="btn btn-danger btn-sm" title="Delete">
